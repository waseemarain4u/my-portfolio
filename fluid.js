...
Adapted for portfolio use: blue/zinc theme, no GUI, dark-mode aware.
*/
'use strict';
...
// Sync background with current dark-mode state
function syncBackground () {
    const isDark = document.documentElement.classList.contains('dark');
    if (isDark) {
        config.BACK_COLOR = { r: 9, g: 9, b: 11 };  // zinc-950
    } else {
...
// Blue/indigo/cyan palette matching the portfolio's #2563EB accent
function generateColor () {
    const rand = Math.random();
    let h;
    if (rand < 0.55) {
        h = (210 + Math.random() * 38) / 360; // core blues 210-248°
    } else if (rand < 0.80) {
        h = (186 + Math.random() * 24) / 360; // cyan-blue 186-210°
    } else {
